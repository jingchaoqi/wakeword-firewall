...
| 阶段 | 状态 | 内容 |
|---|---|---|
| P0 | ✅ 完成 | 离线验证：召回 94.9%、21x 实时，真实视频 9/9。误报 0 次/32 分钟**是合成语料**，见下 |
| P1 | ✅ 完成 | 预扫描 MVP，已发布 v0.1.0。全链路在真实 Chromium 里跑通（含严格 CSP 页面），CI 从零构建 + 137 项测试。fMP4 解析有单测，但 AAC 解码那一环仍需带 AAC 的 Chrome 验 |
| P2 | 🚧 进行中 | 实时兜底 + 上架应用商店。**真实素材的误报率仍是空白**，这是上架前最该补的 |
| P3 | 开放式 | 社区词库、拼音模糊层移植进扩展、Firefox 版 |
...
extension/              Chrome MV3 扩展本体
  ...
  test/
    ...
    text2token-test.js  中文转 token 单测，外加两处副本的一致性断言
                        （33 项，纯 Node，不用浏览器）
    bundled-test.js     引擎已内置时的引导页：自动判定通过，且真跑一遍自检
                        并检查它说了什么（14 项）
    onboarding-test.js  引导页：引擎未装时该说清楚差什么（自己造这个场景，8 项）
    stats-test.js       统计、徽标、设置面板、三档主题的对比度（51 项）
...
已经因此出过两次问题，都是悄悄走偏、没有任何报错：

**一、词表有两份。** `extension/keywords.txt` 和 `p0/keywords.txt`。
往扩展加了「你好悠悠」和三个近音变体，p0 那份一个都没跟上。
要紧之处在于：上面让贡献者用 `p0/scan.py` 测真实素材的误报率，
而它测的根本不是扩展实际发布的那份词表——少三个变体，误报率会偏低，
正好在最需要真实数据的指标上骗自己。

**二、版本号有两份。** `extension/manifest.json` 和 `extension/package.json`。
CI 只校验前者跟 git tag 是否一致，后者落后了不会有任何人发现。

两处现在都有断言（`text2token-test.js`，逐字节 / 逐字段比对），改一边忘了另一边
会当场红。**再加第三份之前先想想能不能不加。**
...
- [ ] 实时兜底路径（`chrome.tabCapture` + offscreen 文档），覆盖直播和预扫描
      ...
      **注意**：在 content script 的 ISOLATED world 里建 `AudioContext` 会命中
      ...
      offscreen 文档
- [x] ~~DRM / 杜比音轨 / worker-MSE 的检测与优雅降级~~ —— **已完成**。三种情况
      `main-world.js` 都会报 `ww:blind`，工具栏图标角上出现 `!`，设置面板顶部
      说明具体原因，页面上还可选弹一条提示（默认关）。不会静默失效。
- [x] ~~去站点化：只要用 MSE 就能工作，不再针对单个播放器适配~~ —— **已完成**。
      全项目没有任何站点特判，hook 的是 `MediaSource`/`SourceBuffer` 本身，
      容器按内容嗅探（WebM/Opus 或 fMP4），跟是哪个站没关系。
