...
一共五步。**只有第 3 步费时间（20–40 分钟），而且一辈子只用做一次。**
...
| 徽标 | 含义 |
|---|---|
| 数字（青色） | **这一页**已经静音了几次唤醒词 |
| `!`（红棕色） | **这一页抓不到音频**——DRM、杜比音轨，或播放器把 MSE 放在了 Worker 里 |
| `3!` | 拦到过 3 次，但这一页**还有另一条轨挡不住**（比如画中画是 DRM） |
| 无 | 这一页还没有检测到需要处理的音频 |

数字是**按标签页**算的，换页自动清零。鼠标悬停在图标上有一句话说明。

不会静默失效：当前页不支持拦截时，图标角上**一定**有 `!`——这条跟任何开关都无关。
页面上要不要也弹一条悬浮提示，是可选的，默认关（DRM 站每次进都会触发，太吵）。

**点图标打开设置面板**：

顶部是这一页的战果，按唤醒词分开列；切到「历史累计」能看跨会话的总账，
也是按词分的——想知道哪个词最常被视频念到就看这里。这一页要是有挡不住的内容，
上面会有一条橙色说明告诉你为什么。
...
- **两个提示开关** —— 拦截时弹不弹、当前页不支持时弹不弹（后者默认关）。
  注意后者只管**页面上**弹不弹；不管它开着还是关着，工具栏图标角上都会有 `!`
- **外观** —— 透明度和背景色，提示条和设置面板共用一套，改完立刻生效不用刷新。
  文字色按背景亮度自动挑（浅底自动换深字），所以调不出看不清的配色。
  旁边有「恢复默认」
- **唤醒词表** —— 直接编辑，保存后刷新页面生效

### 它要哪些权限，各自为什么

装的时候 Chrome 会列一串权限。一个号称「零网络请求」的东西要这么多权限，
本来就该解释清楚——逐条对照 `extension/manifest.json`：

| 权限 | 干什么用 | 不给会怎样 |
|---|---|---|
| `<all_urls>` | 在任意视频页注入 hook。**唤醒词出现在哪个站点是不可预知的**，没法列白名单 | 只能在预先列出的站点上工作 |
| `storage` | 存词表、设置、统计。`storage.local` 存历史累计，`storage.session` 存本页计数 | 设置和统计都留不住 |
| `unlimitedStorage` | 引擎是用户自己装的时候，25 MB 的 wasm 要存在本地 | 自装引擎那条路走不通（引擎已内置的包不受影响） |
| `tabs` | 设置面板要知道当前标签页是哪个，才能把统计对上、点「刷新页面」 | 面板拿不到当前页 |
| `offscreen` | 检测跑在 offscreen 文档里。内容脚本建的 Worker 继承页面的 CSP，大站会把它拦死 | 检测在严格 CSP 的站点上起不来 |

**没有** `webRequest`、`cookies`、`history`、`tabCapture`，也没有任何网络访问——
模型和引擎都打在包里，MV3 本来也禁止远程加载代码。

> 装过老版本的话会注意到 `scripting` 没了：它以前只被引导页的「兼容性探测」
> 用着，那一步已经撤掉（开发者工具，对普通用户没意义），权限跟着一起去了。
...
前四种**工具栏图标角上一定会出现 `!`，不会静默失效**（页面上要不要也弹一条，
在设置面板里勾，默认不弹）。

最后两种是**静默的**——防护正常跑着，只是这个词不在它认识的范围里，图标不会变、
页面上也不会有任何提示。这也是[屏蔽词的边界](#屏蔽词的边界)那一节值得读完的原因。
...
**图标角上一直是 `!`**
把鼠标停在图标上，或点开设置面板，顶部会说明是哪一种情况（DRM / 播放器把
MediaSource 放进了 Worker / Chrome 解不了这个音频编码）。

如果是 Worker 那种，可以在视频页的控制台自己复核：
