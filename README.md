...
**加新词最简单的办法：打开设置面板，在「唤醒词表」上方输入中文，点添加。**

扩展会自动转成上面那种 token 序列并追加进词表，转换结果会显示出来给你确认，
最后点「保存词表」生效（刷新页面后对新命中起作用）。

转换在本地做，不联网。覆盖 20840 个常用汉字。转不了的字会明确告诉你是哪个、
为什么——通常是生僻字，或者输入里混了英文/数字。

> **多音字取的是最常见读音。** 比如「行」默认按 xíng。如果你要的是另一个读音，
> 添加后直接在词表框里改那一行的 token 即可，格式是「声母 韵母(带声调)」。
