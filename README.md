...
> **误报那一行要打折看。** 它是在合成语料上测的，而真实视频有背景音乐、多人
> 同时说话、压缩失真——这些恰恰是误报的温床。**误报是这类插件唯一会致死的
> 问题**，用户被误伤两次就会卸载。真实素材上的误报率目前**还没有数据**，
> 这是[最需要帮忙的事](CONTRIBUTING.md#最需要帮忙的三件事)之一：
> `python3 p0/scan.py 你的素材目录/ --json result.json`。
